...
"""
    ...
    async def handle_connection_impl(
        ...
        segment = 0
        timestamp = 0.0
        lastReconizedTime = 0.0
        lastTranslate = [""]

        while True:
            samples = await self.recv_audio_samples(socket)
            logging.info(f"recv_audio_samples: {len(samples)}")
            if samples is None:
                break

            currentTimestamp = float(samples[:1][0])
            logging.info(f"currentTimestamp: {currentTimestamp}")
            samples = samples[1:]
            ...
            while self.recognizer.is_ready(stream):
                ...
                if currentTimestamp - lastReconizedTime > 0.5:
                    lastReconizedTime = currentTimestamp
                    result_cn = self.translate(result) if result else [""]
                    if result: 
                        lastTranslate = result_cn
                        logging.info(result_cn)

                message = {
                    "text": result,
                    "segment": segment,
                    "translatecn": lastTranslate,
                    "startTime": timestamp,
                }
                if self.recognizer.is_endpoint(stream):
                    ...
                    message["translatecn"] = self.translate(result)
                    lastTranslate=[""]
    ...
    def translate(self, result):
        ...
        input_ids = tokenizer(prefix + result, return_tensors="pt")
        generated_tokens = model.generate(**input_ids.to(device))
        result_cn = tokenizer.batch_decode(generated_tokens, skip_special_tokens=True)
        return result_cn
...
model_name = './t5_translate_en_ru_zh_small_1024'
